...
def request(token):
    ...
    update_raw = requests.get(
        "https://api.telegram.org/bot" + token + "/getUpdates?offset=" + str(OFFSET)
    )
...
def console_Polling(token, guitext):
    ...
    if result == "ConnectionError":
        evm.event_log(
            "Connection Error. Reconnection in 30 Seconds",
        ...
        if result["channel_post"]["chat"]["id"] == chatID:
            ...
            evm.event_log(
                "New incoming command: <" + result_text + ">. Sending to Interpreter.",
